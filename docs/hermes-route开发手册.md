...
而"加一条别名"的常规配方是按固定字段写 `hermes config set model_aliases.X '{...}'`（§7.1 就是
...
而这次加窗口/上限只改了 `build_commands()` 一处，备份/重钉/验证/回滚那套机制一行没动。
...
8. **校验只防手滑，不证真伪**：`--set-limits` 会校验取值范围与 `max_tokens ≤ context_length`，但数值对不对仍要靠查证——GLM 5.x 与 5.3 的窗口差 5 倍就是例子（§1.2）。而 `context_length` **无法从 API 探测**（网关 `/v1/models` 不返回窗口大小），只能查文档或已知值。
9. **文档里的 `--no-verify` 是历史残留**：早期文档写过这个参数但脚本从未实现，已于 2026-09-23 从文档中移除。验证是安全网的一部分，不提供跳过开关。
